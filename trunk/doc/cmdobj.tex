...
Calls the Sybase-CT \function{ct_bind()} function passing the \var{num}
and \var{datafmt} arguments.  It returns a tuple containing the Sybase
result code and a \class{DataBuf} object which is used to retrieve
data from the column identified by \var{num}.  The Sybase-CT
\function{ct_bind()} function is called like this:
...
result code.  The \var{type} argument controls determines the type and
