...
def connect_db(ctx, user_name, password):
    ...
    if status != CS_SUCCEED:
        raise SybaseError(conn, "ct_con_props failed")
...
def ProcessTimestamp(cmd, textdata):
    ...
    if status != CS_SUCCEED:
        raise SybaseError(cmd.conn, "ct_describe failed")
...
def updatetextdata(textdata, iodesc, newdata):
    ...
    if status != CS_SUCCEED:
        raise SybaseError(cmd.conn, "ct_data_info failed")
    ...
    for c in newdata:
        ...
        if status != CS_SUCCEED:
            raise SybaseError(cmd.conn, "ct_send_data failed")
...
def fetch_n_print(cmd):
    ...
    while 1:
        ...
        if status != CS_END_ITEM:
            raise SybaseError(cmd.conn, "ct_get_data failed")
